(* Trident 2HR Swing — Tick-Based ODE Model with Reinforced Learning and Fibonacci Logic *)
...
let state_ode genome t (st: state) (tick: tick) (prev_close: float) (cum_pv, cum_vol) high low =
  ...
  let new_vol_ema = ema_ode (2. /. 21.) st.vol_ema tick.volume in
...
let generate_signals ticks genome =
  let rec loop acc st t cum_pv cum_vol high low = function
    | [] -> List.rev acc
    | tick::rest ->
      let prev_close = st.price in
      let high' = max high tick.last in
      let low' = min low tick.last in
      let st', (new_pv, new_vol) = state_ode genome t st tick prev_close (cum_pv, cum_vol) high' low' in
      let long_signal =
      ...
      in
      loop (match signal with Some s -> s::acc | None -> acc) st' (t +. 1.) new_pv new_vol high' low' rest
  in
  match ticks with
  | [] -> []
  | t0::ts ->
    let init_price = t0.last in
    let init_state = {
      ema_fast = init_price;
      ema_slow = init_price;
      vwap = init_price;
    ...
    loop [] init_state 0.0 0.0 0.0 init_price init_price (t0::ts)
